...
矩阵完成后当时的策略改为信任 Director API：API 返回成功就直接调用 `L4D2_SpawnSpecial`，不再复算有向路径、职业距离、可见性、stuck、预测落点、伤害区域或最小直线距离。实体生成后只确认类型、存活状态和短暂稳定存在，不做位置安全复核。该段是 2026-08-06 历史行为，不代表现行 2000u Director 硬上限。
