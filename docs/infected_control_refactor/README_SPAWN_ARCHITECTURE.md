...
- worker 从输出集合排除中心点距任意存活生还者小于 250 单位的 Nav，再按目标眼位的空间距离升序排序。250 范围内的 Nav 仍可作为图路径的中间节点，否则会切断所有到目标附近的路径，但不会作为刷点候选返回。
- SourcePawn 按当前职业距离带加 128 单位边界余量分页取结果。扩展不可用或图不完整时才从索引 0 扫描全量 Nav；快照暂未完成时返回 pending，不会偷偷切回 Flow 桶或全图抢跑。
...
- 有向图 worker 一次完成反向可达、全队 250 单位排除和目标距离排序；SourcePawn 不再按 Flow 桶枚举候选。
...
- `inf_spawn_perf_stats 1` 还会把每次实际实体生成的波号、职业、普通/传送/导演模式、开波后耗时、生成调用耗时、请求/实际坐标和位置评分写入同一日志；下一波或回合结束时输出按职业、模式、耗时和分数聚合的波次总结。导演兜底没有评分时明确写 `N/A`。
