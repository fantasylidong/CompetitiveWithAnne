...
8. 最近刷点硬防叠：生成成功后的即时实际落点保留 `inf_spawn_sep_ttl`（默认 1.5 秒），期间按 `inf_spawn_sep_radius`（默认 100）硬拒绝邻近请求点。该半径是固定值，不随特感上限或波内占用缩放——这一条只回答「能不能刷在这」，方向和聚集程度交给评分里的核密度项。
...
- 分散度：基础 100 分，只保留核密度一项——对仍在接近途中的特感与最近刷点按 `exp(-d/r)` 累加，扣分 = `inf_spawn_kernel_points`（默认 50）× min(核密度和, 1.5)。已贴脸交战（距任意生还者 <400）的存活特感不计入核源。扇区量化项已删除，方向偏好由连续战术几何分表达。特感上限缩放作用在核半径 `r` 上（`inf_spawn_kernel_radius` × 缩放系数：低特推得开、高特只防叠），不再乘在分值上。
