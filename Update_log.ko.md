...
- `infected_control.smx`를 2026-08-13.1로 올리고 전체 코드 감사에서 발견된 카운트·상태 문제를 수정했습니다. 웨이브 타이밍 계약(킬 페이즈, 독립적인 16초 기본 카운트다운, 상한 없는 Anti-Bait 홀드)의 동작은 그대로입니다.
- 특수 감염자 생존 수 이중 집계를 수정: `RecalcSiCapFromAlive` 같은 전체 재집계가 "검증 대기, 거부되어 킥 대기, 킥 큐에 있는" 임시 클라이언트를 건너뛰어, 텔레포트 킥이나 확인 대기 스폰 중에 클래스/전체 상한이 먼저 차서 웨이브 판단이 왜곡되는 문제를 막았습니다.
...
- 최종 착지 검증이 오래된 시야 스냅샷으로 거리 하드 판정을 먼저 하던 순서 문제를 수정했습니다. `GetNextSpawnTime` native는 킬 페이즈 중 "남은 확인 창 + 완전한 기본 카운트다운"의 상한 추정치를 반환하고, 일시정지 중에도 동결된 시계로 추정을 이어가며 킬 페이즈 시간을 기본 카운트다운에 합산하지 않습니다.
- Anti-Bait 상태 머신 재설계(2026-08-13.2): 산개로 홀드를 해제하려면 지속 확인이 필요합니다. 새 `inf_antibait_spread_confirm`(기본 3초) 동안 대형이 연속으로 산개해 있어야 Pressure가 강등됩니다. "잠깐 벗어났다가 바로 복귀"하는 페이크로는 강화 홀드를 속일 수 없으며, 산개는 정체 타이머도 초기화하지 않습니다. 무장 창 안에서 다시 뭉치고 여전히 정체 중인 팀은 같은 홀드의 연속으로 간주되어 Pressure로 바로 복귀합니다.
