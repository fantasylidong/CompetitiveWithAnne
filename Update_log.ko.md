...
- `ai_tank3`를 1.0.0.7로 업데이트: `OnPlayerRunCmd`가 하위 로직의 반환값을 집계해 반환하도록 하여, 투석 거리 제한·머리 위 막기 대응 강제 투석·연속 점프의 점프 입력이 엔진에서 버려지던 문제를 수정했습니다. 펀치 시 시점 고정은 GameTime 시계로 통일했고, 투석 릴리스 높이는 시퀀스 번호와 열거값의 우연한 일치에 의존하지 않고 애니메이션 액티비티 이름으로 해석하며, `tank_swing_range` 변경 훅이 맵 변경 때마다 중복 등록되지 않도록 했습니다.
- `ai_charger3`를 1.0.1.5로 업데이트: 상태 머신 `transitionTo`에서 이전 상태의 `OnExit`가 (읽기 전에 검사해서) 한 번도 실행되지 않던 문제를 수정하여, 공중 급정지·후방 점프·인위적 돌진 지연 플래그가 상태 전환 시 올바르게 초기화됩니다. `player_spawn` 이벤트는 Post 훅으로 변경했고, 사다리에서 점프/앉기 입력을 지운 뒤 올바르게 `Plugin_Changed`를 반환합니다.
- `ai_boomer_3`를 3.0.5로 업데이트: `L4D_OnVomitedUpon`에 attacker 검증을 추가하여 담즙병 등 attacker가 0인 경우 매번 오류가 발생하던 문제를 수정했습니다. 전환 구토 대기열이 존재하는 동안에는 다시 만들지 않아 강제 구토 재귀로 인한 중복 목표 증가를 방지합니다. 목표 선택의 조기 반환 시 trace 핸들 누수와 강제 구토 루프에서 잘못된 인덱스에 상태를 기록하던 문제도 수정했으며, 최근접 생존자 조회에 0.2초 캐시를 추가해 프레임당 정렬 비용을 줄였습니다.
- `ai_spitter_3`를 3.0.3으로 업데이트: "밀집 지역" 목표 정책이 고정된 목표의 좌표를 잘못 사용하던 복사-붙여넣기 버그(-1 인덱스 오류 포함)를 수정하고, `getTargetByPriority`가 누군가 제압당했거나 생존자가 1명뿐일 때 ArrayList를 누수하던 문제를 수정했습니다. 생존자가 1명만 남아도 진행도 기준으로 목표로 선택되며, 밀집도 거리 누적 셀을 명시적으로 0으로 초기화하고, 항상 거짓이 되는 레이 필터 데드 코드를 제거했습니다.
- 트레이스 비용 최적화(tank3 1.0.0.8 / charger3 1.0.1.6 / boomer3 3.0.6 / spitter3 3.0.4): `ai_path_movement.inc`에 점프 경로 안전 검사 실패 스로틀을 추가했습니다. 벽·단차·위험한 착지점 등 지형으로 인한 검사 실패는 0.1초 쿨다운 동안 재시도하지 않습니다. 네 AI의 지상 점프 시도가 모두 이를 사용하여, 특수 감염자가 지형에 막혀 있을 때의 hull/ray 트레이스 비용을 약 1/6(60틱 기준)로 줄이며 다중 Charger 조합에서 효과가 가장 큽니다. 또한 `AIPathMovement_IsJumpRouteSafe`는 트레이스가 필요 없는 무시야 각도 거부를 hull 트레이스보다 앞으로 옮겨, 거부된 시도는 트레이스를 전혀 발생시키지 않습니다.
