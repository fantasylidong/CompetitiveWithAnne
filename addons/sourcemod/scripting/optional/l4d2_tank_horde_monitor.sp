...
public Action L4D_OnSpawnMob(int &amount)
{
	...
	if (tankInPlay && FindTankClient() && bInfiniteHordeActive){
		...
		if (announcedHordeMax){
			...
			if (announcedHordeResume) {
				iNewAmount += ConsumeDeferredMobCount();
			} else {
				DeferMobCount(amount - iNewAmount);
			}
...
void RestoreDeferredMobCount()
{
	...
	SetPendingMobCount(GetPendingMobCount() + ConsumeDeferredMobCount());
}

int ConsumeDeferredMobCount()
{
	int count = deferredMobCount;
	deferredMobCount = 0;
	deferredMobSavedForTank = false;
	return count;
}
