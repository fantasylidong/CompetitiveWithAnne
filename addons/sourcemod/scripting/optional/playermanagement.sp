...
public void OnPluginStart()
{
	...
	g_cvarBlockInTank		  = CreateConVar("sm_blockspecintank", "0", "Block suvivors from switching to spect while in tank", FCVAR_NONE, true, 0.0, true, 1.0);
...
Action Swap_Cmd(int client, int args)
{
	if (args < 1)
	{
		CReplyToCommand(client, "%t %t: sm_swap <player1> <player2> ... <playerN>", "Tag", "Usage");
		return Plugin_Handled;
	}
...
Action SwapTo_Cmd(int client, int args)
{
	if (args < 2)
	{
		CReplyToCommand(client, "%t %t: sm_swapto <teamnum> <player1> <player2> ... <playerN>\n%d = Spectators, %d = Survivors, %d = Infected", "Tag", "Usage", L4D2Team_Spectator, L4D2Team_Survivor, L4D2Team_Infected);
		CReplyToCommand(client, "%t %t: sm_swapto force <teamnum> <player1> <player2> ... <playerN>\n%d = Spectators, %d = Survivors, %d = Infected", "Tag", "Usage", L4D2Team_Spectator, L4D2Team_Survivor, L4D2Team_Infected);
		return Plugin_Handled;
	}
	...
	{
		CReplyToCommand(client, "%t %t", "Tag", "ValidTeams", L4D2Team_Spectator, L4D2Team_Survivor, L4D2Team_Infected);
		return Plugin_Handled;
	}
