...
public void OnPluginStart()
{
	...
	g_hTankBhopSpeed = CreateConVar("ai_Tank_BhopSpeed", "60.0", "Tank连跳的速度", FCVAR_NONE, true, 0.0);
	g_hTankStopDistance = CreateConVar("ai_Tank_StopDistance", "130", "Tank在距离目标多远位置停下来", FCVAR_NONE, true, 0.0);
	g_hTankBhop = CreateConVar("ai_Tank_Bhop", "1", "是否开启Tank连跳功能：0=关闭，1=开启", FCVAR_NONE, true, 0.0, true, 1.0);
	g_hTankThrow = CreateConVar("ai_Tank_Throw", "1", "是否允许Tank投掷石块：0=关闭，1=开启", FCVAR_NONE, true, 0.0, true, 1.0);
	g_hTankThrowDist = CreateConVar("ai_TankThrowDistance", "450", "Tank距离目标多近允许投掷石块", FCVAR_NONE, true, 0.0);
	g_hTankBlockThrowDist = CreateConVar("ai_TankBlockThrowDistance", "200", "Tank距离目标多近时阻止投掷石块", FCVAR_NONE, true, 0.0);
	g_hTankTarget = CreateConVar("ai_TankTarget", "1", "Tank目标选择：1=最近，2=血量最少，3=血量最多", FCVAR_NONE, true, 1.0, true, 3.0);
	g_hTreeDetect = CreateConVar("ai_TankTreeDetect", "1", "生还者与Tank进行秦王绕柱时进行的操作：0=关闭此项，1=切换目标，2=将Tank传送至绕树的生还者后", FCVAR_NONE, true, 0.0, true, 2.0);
	g_hTreeNewTarget = CreateConVar("ai_TankTreeNewTargetDistance", "300", "Tank记录绕树生还者并选择新目标后，距离新目标多近重置绕树目标记录", FCVAR_NONE, true, 0.0);
	g_hTankAirAngles = CreateConVar("ai_TankAirAngles", "60.0", "Tank在空中的速度向量与到生还者的方向向量夹角大于这个值停止连跳", FCVAR_NONE, true, 0.0, true, 90.0);
	g_hTankConsume = CreateConVar("ai_TankConsume", "0", "是否开启Tank消耗功能", FCVAR_NONE, true, 0.0, true, 1.0);
	g_hTankConsumeHeight = CreateConVar("ai_TankConsumeHeight", "100", "Tank进行消耗时将会优先选择高于这个高度的位置，如无则随机选位", FCVAR_NONE, true, 0.0);
	g_hTankConsumLimit = CreateConVar("ai_TankConsumeLimit", "0", "感染者团队少于等于当前刷特数量 - 这个值的特感时", FCVAR_NONE, true, 0.0);
	g_hTankConsumeRaidus = CreateConVar("ai_TankConsumeRaidus", "80.0", "Tank消耗位置的范围，从中心坐标以这个半径画圆", FCVAR_NONE, true, 0.0);
	g_hTankAttackVomitedNum = CreateConVar("ai_TankAttackVomitedNum", "1", "如果有这个数量的生还者被Boomer喷吐到，正在进行消耗的Tank将会攻击", FCVAR_NONE, true, 0.0);
	g_hVomitCanInstantAttack = CreateConVar("ai_TankVomitCanInstantAttack", "1", "是否开启固定数量生还者被喷吐后Tank立刻攻击", FCVAR_NONE, true, 0.0, true, 1.0);
	g_hVomitAttackInterval = CreateConVar("ai_TankVomitAttackInterval", "20.0", "从开始被喷且Tank允许攻击时开始，这个时间内Tank允许攻击", FCVAR_NONE, true, 0.0);
	g_hTeleportForwardPercent = CreateConVar("ai_TankTeleportForwardPercent", "10", "Tank开始消耗时，记录此时生还者行进距离x，生还者前压超过x + 这个值时，Tank会传送至生还者处进行压制", FCVAR_NONE, true, 0.0);
	g_hTankConsumeLimitNum = CreateConVar("ai_TankConsumeLimitNum", "5", "Tank最多进行消耗的次数（找到一次消耗位并抵达消耗位算 1 次消耗）", FCVAR_NONE, true, 0.0);
	g_hTankConsumeType = CreateConVar("ai_TankConsumeType", "6", "Tank进行消耗将会按照哪种特感类型找位：1=Smoker，2=Boomer，3=Hunter，4=Spitter，5=Jockey，6=Charger，8=Tank", FCVAR_NONE, true, 1.0, true, 8.0);
	g_hTankRetreatAirAngles = CreateConVar("ai_TankRetreatAirAngles", "75.0", "Tank在回避的连跳过程中视角与速度超过这个值将会停止连跳", FCVAR_NONE, true, 0.0);
	g_hTankConsumeAction = CreateConVar("ai_TankConsumeAction", "2", "Tank在消耗范围内将会：1=冰冻，2=可活动但不允许超出消耗范围", FCVAR_NONE, true, 1.0, true, 2.0);
	g_hTankConsumeDamagePercent = CreateConVar("ai_TankConsumeDamagePercent", "50", "Tank在消耗过程中只会受到这个百分比的伤害", FCVAR_NONE, true, 0.0, true, 100.0);
	// 2022-4-7新增
	g_hTankForceAttackDistance = CreateConVar("ai_TankForceAttackDistance", "300", "Tank在离最近生还这个距离时即使可以消耗也不会当着生还的面回避生还（强制压制）", FCVAR_NONE, true, 0.0);
	g_hTankAttackIncapped = CreateConVar("ai_TankIncappedCount", "1", "强制压制时，需要拍倒这个数量的生还者才允许继续检测是否可以消耗", FCVAR_NONE, true, 0.0);
	g_hTankConsumeHealthLimit = CreateConVar("ai_TankConsumeHealthLimit", "1200", "Tank在少于这么多血量时不会消耗", FCVAR_NONE, true, 0.0);
	g_hTankConsumeValidRaidus = CreateConVar("ai_TankConsumeValidRaidus", "1800", "Tank在当前消耗位上，如果当前消耗位不能直视生还，则以这个半径重新找位", FCVAR_NONE, true, 0.0);
	g_hTankConsumeDistance = CreateConVar("ai_TankConsumeDistance", "1200", "Tank消耗找位的位置必须离生还者大于这个距离", FCVAR_NONE, true, 0.0);
	// 其他 Cvar
