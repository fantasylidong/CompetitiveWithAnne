...
public void OnPluginStart()
{
	...
	g_hPathLaneOffset = CreateConVar("ai_boomer3_path_lane_offset", "10.0", "路径连跳的最大稳定侧向分流距离", CVAR_FLAG, true, 0.0, true, 40.0);
...
public void OnAllPluginsLoaded()
{
	if (!LibraryExists(ANNE_NEXTBOT_LIBRARY) || !AnneNextBot_IsActive())
		SetFailState("ai_boomer3 requires anne_nextbot");
	if (!AIPathSnapshot_Setup())
		SetFailState("ai_boomer3 failed to prepare path snapshot calls");
	if (!AnneNextBot_SetPathConsumer(AnneNextBotPathConsumer_Boomer, true))
		SetFailState("ai_boomer3 failed to enable the anne_nextbot path broker");
}
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3])
{
	...
	// === 初次对准 + 贴脸仰角兜底（修正 C） =========================================
	if (!pathAirFacing && has_sight && IsValidSurvivor(target) && !in_bile_interval[client] && targetList[client].Length < 1)
	{
		if (targetDist <= g_hVomitRange.FloatValue)
		{
			ComputeAimAngles(client, target, aim_angles, AimEye);

				// 保留 26-07 AI2 的喷吐仰角修复。
	...
	if (!pathAirFacing && targetList[client].Length >= 1 && !in_bile_interval[client] && g_hTurnVision.BoolValue)
	{
		/* 当前喷吐目标索引小于胖子目标数 */
		...
		&& cur_speed > 160.0 && !AIPathMovement_IsRouteCheckThrottled(client))
	{
		...
		if (canUsePath)
		{
			float maxEstimateDist = cur_speed * 0.42;
			if (maxEstimateDist < 110.0)
				maxEstimateDist = 110.0;
