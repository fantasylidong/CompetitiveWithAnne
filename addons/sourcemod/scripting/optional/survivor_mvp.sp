...
#define ANNE_COMMON_INFECTED_BONUS 20
...
new                 iAnneGotKills[MAXPLAYERS + 1];              // SI kills for AnneHappy compact stats
new                 iAnneGotCommon[MAXPLAYERS + 1];             // common kills for AnneHappy compact stats
new                 iAnneDidDamage[MAXPLAYERS + 1];             // capped SI damage for AnneHappy compact stats
new                 iAnneDidFF[MAXPLAYERS + 1];                 // friendly fire dealt for AnneHappy compact stats
new                 iAnneTookFF[MAXPLAYERS + 1];                // friendly fire received for AnneHappy compact stats
...
public OnClientPutInServer(client)
{
    ...
    if (strcmp(tmpBuffer, sClientName[client], true) != 0)
    {
        ...
        iDidFF[client] = 0;
        iAnneGotKills[client] = 0;
        iAnneGotCommon[client] = 0;
        iAnneDidDamage[client] = 0;
        iAnneDidFF[client] = 0;
        iAnneTookFF[client] = 0;
...
Action SurvivorMVP_Cmd(client, args)
{
    ...
    GetMVPString(client, printBuffer, sizeof(printBuffer));
...
Action:delayedMVPPrint(Handle:timer)
{
    ...
    CreateTimer(0.2, delayedAnneHappyMVPPrint);
}
...
Action:delayedAnneHappyMVPPrint(Handle:timer)
{
    PrintAnneHappyMVPReport(0);
}
...
void PrintLoserz(bool:bSolo, client)
{
    ...
    if (iTotalDamageAll > 0)
    {
        ...
        for (new i = 0; i <= 2; i++)
        {
            if (IsClientAndInGame(mvp_SI_losers[i]) && !IsFakeClient(mvp_SI_losers[i]) && (!bSolo || mvp_SI_losers[i] == client))
            {
                CPrintToChat(mvp_SI_losers[i], "%t", "YourRankSI",
                    (i + 2),
    ...
    if (iTotalCommon > 0)
    {
        ...
        for (new i = 0; i <= 2; i++)
        {
            if (IsClientAndInGame(mvp_CI_losers[i]) && !IsFakeClient(mvp_CI_losers[i]) && (!bSolo || mvp_CI_losers[i] == client))
            {
                CPrintToChat(mvp_CI_losers[i], "%t", "YourRankCI",
                    (i + 2),
    ...
    if (iTotalFF > 0)
    {
        ...
        for (new i = 0; i <= 2; i++)
        {
            if (IsClientAndInGame(mvp_FF_losers[i]) && !IsFakeClient(mvp_FF_losers[i]) && (!bSolo || mvp_FF_losers[i] == client))
            {
                CPrintToChat(mvp_FF_losers[i], "%t", "YourRankFF",
                    (i + 2),
...
void PrintAnneHappyMVPReport(client)
{
    ...
    {
        if (IsClientAndInGame(i) && GetClientTeam(i) == TEAM_SURVIVOR)
        {
            players_clients[players++] = i;
        }
    }

    SortCustom1D(players_clients, players, SortAnneHappyMVPByDamageDesc);
    ...
    {
        new target = players_clients[i];
        if (!IsClientAndInGame(target) || GetClientTeam(target) != TEAM_SURVIVOR)
        {
            continue;
        }
...
public SortAnneHappyMVPByDamageDesc(elem1, elem2, const array[], Handle:hndl)
{
    new score1 = iAnneDidDamage[elem1] + iAnneGotCommon[elem1] * ANNE_COMMON_INFECTED_BONUS;
    new score2 = iAnneDidDamage[elem2] + iAnneGotCommon[elem2] * ANNE_COMMON_INFECTED_BONUS;
...
GetAnneHappyCappedSIDamage(zombieClass, damageDone)
{
    switch (zombieClass)
    {
        case ZC_SMOKER:
        {
            return damageDone > 250 ? 250 : damageDone;
        }
        case ZC_BOOMER:
        {
            return damageDone > 50 ? 50 : damageDone;
        }
        case ZC_HUNTER:
        {
            return damageDone > 250 ? 250 : damageDone;
        ...
        case ZC_SPITTER:
        {
            return damageDone > 100 ? 100 : damageDone;
        }
        case ZC_JOCKEY:
        {
            return damageDone > 325 ? 325 : damageDone;
        }
        case ZC_CHARGER:
        {
            return damageDone > 600 ? 600 : damageDone;
        }
    }
...
void PlayerHurt_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if (victimId && attackerId && IsClientAndInGame(victim) && IsClientAndInGame(attacker))
    {
        ...
        if (GetClientTeam(attacker) == TEAM_SURVIVOR && GetClientTeam(victim) == TEAM_INFECTED)
        {
            ...
            {
                iAnneDidDamage[attacker] += GetAnneHappyCappedSIDamage(zombieClass, damageDone);
        ...
        else if (GetClientTeam(attacker) == TEAM_SURVIVOR && GetClientTeam(victim) == TEAM_SURVIVOR && bTrackFF && !L4D_IsPlayerIncapacitated(victim))                // survivor on survivor action == FF
        {
            if (bRUPLive || bPlayerLeftStartArea) {
                ...
                iAnneDidFF[attacker] += damageDone;
                iAnneTookFF[victim] += damageDone;
                iTotalFF += damageDone;
            }
        }
...
*      ======================
*/
void GetMVPDisplayName(mvp, langClient, String:buffer[], maxlen)
{
    buffer[0] = '\0';
    if (mvp <= 0)
    {
    ...
    {
        GetClientName(mvp, buffer, maxlen);
        if (IsFakeClient(mvp))
        {
            decl String:botTag[32];
            Format(botTag, sizeof(botTag), " {default}%T", "BotName", langClient);
            StrCat(buffer, maxlen, botTag);
        }
    }
    else
    {
        strcopy(buffer, maxlen, sClientName[mvp]);
    }
}

void GetMVPString(langClient, char[] printBuffer, const int iSize)
{
    ...
    SetGlobalTransTarget(langClient);
    
    if (!(iBrevityFlags & BREV_SI))
    {
        mvp_SI = findMVPSI();
        GetMVPDisplayName(mvp_SI, langClient, mvp_SI_name, sizeof(mvp_SI_name));
    }
    
    if (!(iBrevityFlags & BREV_CI))
    {
        mvp_Common = findMVPCommon();
        GetMVPDisplayName(mvp_Common, langClient, mvp_Common_name, sizeof(mvp_Common_name));
    }
    
    if (!(iBrevityFlags & BREV_FF) && bTrackFF)
    {
        mvp_FF = findLVPFF();
        GetMVPDisplayName(mvp_FF, langClient, mvp_FF_name, sizeof(mvp_FF_name));
    }
    ...
    else
    {
        ...
        if (!(iBrevityFlags & BREV_CI))
        {
            if (mvp_Common > 0)
            {
                if (iBrevityFlags & BREV_PERCENT) 
                {
                    Format(tmpBuffer, sizeof(tmpBuffer), "%t %t\n", "Tag", "ReportCI_Absolute",
                        mvp_Common_name, iGotCommon[mvp_Common]);
                } 
                else if (iBrevityFlags & BREV_ABSOLUTE) 
                {
                    Format(tmpBuffer, sizeof(tmpBuffer), "%t %t\n", "Tag", "ReportCI_Percent",
                        mvp_Common_name,
    ...
    if (!(iBrevityFlags & BREV_FF) && bTrackFF)
    {
        ...
        else
        {
            if (iBrevityFlags & BREV_PERCENT) 
            {
                Format(tmpBuffer, sizeof(tmpBuffer), "%t %t\n", "TagLVP", "ReportFF_Absolute",
                    mvp_FF_name, iDidFF[mvp_FF]);
            } 
            else if (iBrevityFlags & BREV_ABSOLUTE) 
            {
                Format(tmpBuffer, sizeof(tmpBuffer), "%t %t\n", "TagLVP", "ReportFF_Percent",
                    mvp_FF_name,
                    (float(iDidFF[mvp_FF]) / float(iTotalFF)) * 100.0);
            } 
            else 
            {
                Format(tmpBuffer, sizeof(tmpBuffer), "%t %t\n", "TagLVP", "ReportFF_Full",
                    mvp_FF_name,
                    iDidFF[mvp_FF],
                    (float(iDidFF[mvp_FF]) / float(iTotalFF)) * 100.0
                );
