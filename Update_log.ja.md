...
- `ai_tank3` を 1.0.0.7 へ更新：`OnPlayerRunCmd` がサブロジックの戻り値を集約して返すようになり、投石距離制限・頭上ブロック対策の強制投石・連続ジャンプのジャンプ入力がエンジンに破棄される問題を修正しました。パンチ時の視点ロックは GameTime 時計に統一し、投石のリリース高さはシーケンス番号と列挙値の偶然の一致に頼らずアクティビティ名で解決し、`tank_swing_range` の変更フックがマップ変更のたびに重複登録されないようにしました。
- `ai_charger3` を 1.0.1.5 へ更新：状態機械の `transitionTo` で旧状態の `OnExit` が（読み込み前に判定していたため）一度も実行されない問題を修正し、空中急停止・後方ジャンプ・人為的な突進遅延フラグが状態遷移時に正しくリセットされるようになりました。`player_spawn` イベントは Post フックに変更し、はしご上でジャンプ/しゃがみ入力を消した際は正しく `Plugin_Changed` を返します。
- `ai_boomer_3` を 3.0.5 へ更新：`L4D_OnVomitedUpon` に attacker の検証を追加し、胆汁瓶など attacker が 0 のケースで毎回エラーが出る問題を修正しました。転向吐しゃキューが存在する間は再構築せず、強制吐しゃの再帰による重複ターゲットの膨張を防ぎます。ターゲット選択の早期リターンでの trace ハンドルリークと、強制吐しゃループでの誤ったインデックスへの状態設定も修正し、最近接生存者の検索に 0.2 秒キャッシュを追加して毎フレームのソートコストを削減しました。
- `ai_spitter_3` を 3.0.3 へ更新：「人が密集した場所」ターゲット方針が拘束ターゲットの座標を誤用していたコピペバグ（-1 インデックスのエラーを含む）を修正し、`getTargetByPriority` が誰かが拘束中または生存者が 1 人だけの場合に ArrayList をリークする問題を修正しました。生存者が 1 人だけでも進行度でターゲットとして選ばれるようになり、密集度の距離集計セルを明示的にゼロ初期化し、常に偽になるレイフィルターのデッドコードを削除しました。
- トレースコスト最適化（tank3 1.0.0.8 / charger3 1.0.1.6 / boomer3 3.0.6 / spitter3 3.0.4）：`ai_path_movement.inc` にジャンプ経路安全確認の失敗スロットルを追加しました。壁・段差・危険な着地点など地形起因の失敗は 0.1 秒のクールダウン内では再試行しません。4 つの AI の地上ジャンプ試行がすべてこれを利用し、地形にブロックされた際の hull/ray トレースコストを約 1/6（60 tick 時）に削減します。複数 Charger 編成で最も効果的です。また `AIPathMovement_IsJumpRouteSafe` はトレース不要の視界なし角度拒否を hull トレースの前に移動し、拒否された試行はトレースを一切発行しなくなりました。
