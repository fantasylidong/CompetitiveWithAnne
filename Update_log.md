...
- `l4d2_hitsound.smx` 音效套装新增两个可选配置键，用于自定义“何时响”：`headshot_kill` 为爆头击杀指定专用音效（留空则沿用 headshot 音，即旧行为）；`stack 1` 让该套装每个命中/击杀事件立即独立播放，可同帧叠加，不再被“同帧只播一条、击杀>爆头>命中”的优先级合并吞掉。
- 套装 9“原叮投票插件”借此复刻老 killsound(Dingshot) 的触发手感：爆头命中=叮（littlereward）、爆头击杀=钟声（bell_normal）、非爆头命中/击杀不响；霰弹一枪多杀恢复逐次叮声叠加。其他套装行为不变。
