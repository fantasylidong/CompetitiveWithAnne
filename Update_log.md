...
- `infected_control.smx` 升级到 2026-08-13.1，修复一轮全量代码审计发现的计数与状态问题；波次时序契约（击杀阶段、独立 16 秒基础倒计时、Anti-Bait 无上限保持）行为不变。
- 修复特感存活计数双重统计：`RecalcSiCapFromAlive` 等全图清点现在跳过“待校验、已拒绝待踢出、踢出队列中”的临时实体，避免传送踢出或待确认刷出期间职业/总量上限被提前打满，导致少刷或波次判断失真。
...
- 修复最终落点校验先用旧眼位快照做距离硬判定的顺序问题；`GetNextSpawnTime` native 在击杀阶段返回“剩余检查窗口 + 完整基础倒计时”的上界估算，暂停时无记录也按冻结时钟继续估算，不再把击杀阶段耗时折算进基础倒计时。
- Anti-Bait 状态机重设计（2026-08-13.2）：散开解除保持改为需要持续确认——新增 `inf_antibait_spread_confirm`（默认 3 秒），队形必须连续散开这么久才会从 Pressure 降级；“稍微走一步马上回来”的假走位不再能骗掉强化保持，散开也不再重置停滞时钟。武装窗口内重新集结且仍停滞的队伍会直接重返 Pressure，视作同一次保持的延续。
